...
function Groupes() {
  ...
  const [formProjet, setFormProjet] = useState({ nom: '', description: '', dateLimite: '' })
  ...
  const handleCreateProjet = async () => {
    ...
    try {
      const res = await api.post(`/api/groupes/${groupeActif.id}/projets`, formProjet)
  ...
  const handleRejoindre = async () => {
    ...
    setLoading(true)
    try {
      const res = await api.post('/api/groupes/rejoindre', { code_invitation: codeRejoindre })
  ...
  <div className="groupe-projets">
    <div className="projets-header">
      ...
      <button className="btn-rejoindre" onClick={() => setShowProjetModal(true)}>
        + Nouveau projet
      </button>
    </div>
              ...
              {onglet === 'rejoindre' && (
                <>
                  ...
                  <div className="modal-field">
                    ...
                    <input type="text" placeholder="Ex: A1B2C3" value={codeRejoindre} onChange={e => setCodeRejoindre(e.target.value)} />
                  </div>
                  <div className="modal-actions">
                    <button className="btn-annuler" onClick={() => setShowModal(false)}>Annuler</button>
        ...
        {showProjetModal && (
          <div className="modal-overlay" onClick={() => setShowProjetModal(false)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="modal-field">
                <label>Nom *</label>
                <input type="text" value={formProjet.nom} onChange={e => setFormProjet({ ...formProjet, nom: e.target.value })} placeholder="Nom du projet" />
              </div>
              <div className="modal-field">
                <label>Description</label>
                <textarea value={formProjet.description} onChange={e => setFormProjet({ ...formProjet, description: e.target.value })} placeholder="Description..." />
              </div>
              <div className="modal-field">
                <label>Date limite</label>
                <input type="date" value={formProjet.dateLimite} onChange={e => setFormProjet({ ...formProjet, dateLimite: e.target.value })} />
              </div>
              <div className="modal-actions">
                <button className="btn-annuler" onClick={() => setShowProjetModal(false)}>Annuler</button>
                <button className="btn-rejoindre" onClick={handleCreateProjet}>Créer</button>
              </div>
            </div>
          </div>
        )}
